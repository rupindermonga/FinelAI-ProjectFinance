...
def build_extraction_prompt(columns: List[ColumnConfig], categories: List[CategoryConfig] = None, corrections: list = None, cost_categories: list = None) -> str:
    ...
    if cost_categories:
        ...
        fields_desc["cost_category"] = (
            ...
            "1. Hours/perdiem/travel/wages only → Payroll. "
            "2. OLT/ONT/chassis/optical modules/transceivers/network equipment → Electronics. "
            "3. Make Ready (pole preparation BEFORE fiber): "
            "   Bell permits/utility permits; guard cable (U-guard, U9, cable protector); "
            "   anchor rod/anchor plate/guy hardware; hydro make-ready tasks; "
            "   pole transfers; clearing conflicts; engineering/design studies only. "
            "4. Fiber Build (everything else — the actual construction): "
            "   Plow/bore/HDD/directional bore/trench/hydrovac; "
            "   new strand/overlash/aerial strand; pole planting; "
            "   conduit supply or install; vault/handhole install; "
            "   down guy/guy guard INSTALLATION (vs supply of hardware); "
            "   drops; splicing; supply-and-install work; civil installation; "
            "   site utilities (hydro/electricity bills); mobilization/travel for field crews. "
            "NOTE: 'Material' is almost never correct for this project. Conduit supply → Fiber Build. "
            "Guard cable/anchor hardware supply → Make Ready. When unsure → Fiber Build. "
            "(string, required)"
        )
