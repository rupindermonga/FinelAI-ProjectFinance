...
_BOOKS_DB_PATH = os.getenv("BOOKS_DB_PATH", "")


def _get_books_conn():
    if not _BOOKS_DB_PATH or not os.path.exists(_BOOKS_DB_PATH):
        raise HTTPException(
            status_code=503,
            detail="Books integration not configured — set BOOKS_DB_PATH on the server.",
        )
    try:
        conn = sqlite3.connect(_BOOKS_DB_PATH, timeout=5, check_same_thread=False)
...
def preview_books_transactions(
    ...
    """
    conn = _get_books_conn()
    db_session: Session = next(get_db())  # lightweight read; we close it ourselves
    try:
